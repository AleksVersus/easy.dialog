...
**ВНИМАНИЕ!!! Подключение ролей ещё не реализовано в генераторе! TODO: подключение ролей реализует механизм наследования для ролей, таким образом можно ссылаться на роли из других диалогов и не использовать их повторно.**
...
По сути обёртки представляют собой html-разметку, в которую заворачиваются фразы героя или NPC. Однако, реализованы обёртки, как обычный код QSP, что позволяет нам довольно гибко извлекать данные из Роли, оборачивать в любую разметку, форматируя таким образом диалог, как нам удобно.

Как уже было сказано в разделе [Назначение ролей. Форматирование реплик](), обёртки прописываются в роль с помощью тегов `<wrap.btn></wrap.btn>` и `<wrap.frase></wrap.frase>`
