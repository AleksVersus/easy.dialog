...
Вы в любой момент можете вывести в окно дополнительного описания значение любой из этих переменных, если подключите встроенный в модуль отладчик и воспользуетесь командой для строки ввода: `eid INDEX_DIALOG_VALUE=`. Вместо `INDEX_DIALOG_VALUE` нужно указать текстовый индекс одной из ячеек массива `DIALOG_VALUE`, перчисленных в разделе [**Значения DIALOG_VALUE**]():
